...
} = require("../rules/courseStatusRules");
...
const getThumbnailObjectPath = (thumbnail) => {
  ...
  try {
    ...
    return decodeURIComponent(
      pathname.slice(markerIndex + PUBLIC_OBJECT_MARKER.length),
    );
  } catch {
    return null;
  }
...
const validateCoursePayload = async (payload, { partial = false } = {}) => {
  ...
  if (!partial || data.title !== undefined) {
    ...
    if (title.length < 3 || title.length > 200) {
      throw new AppError(
        400,
        "Course title must be between 3 and 200 characters.",
      );
    }
    data.title = title;
  }
  ...
  if (data.thumbnail !== undefined) {
    ...
    const isLocalCourseThumbnail =
      data.thumbnail?.startsWith("/uploads/courses/");
  ...
  if (!partial || data.price !== undefined) {
    ...
    if (!Number.isFinite(price) || price < 0 || price > 99999999.99) {
      throw new AppError(
        400,
        "Course price must be between 0 and 99,999,999.99.",
      );
    }
...
const listOwnedCourses = async (instructorId, filters = {}) => {
  ...
  if (status) {
    if (!COURSE_STATUSES.has(status))
      throw new AppError(400, "Invalid course status filter.");
    where.status = status;
  }
  ...
  const [{ count, rows: courses }, totalCourses, hiddenCourses] =
    await Promise.all([
      ...
      Course.count({
        where: { instructor_id: instructorId, status: "Hidden" },
      }),
    ]);
  ...
  try {
    ...
    if (uploadedThumbnail) {
      removeThumbnail(uploadedThumbnail.objectPath).catch((cleanupError) => {
        console.error(
          "Could not remove the unused course thumbnail:",
          cleanupError,
        );
      });
    }
...
exports.update = asyncHandler(async (req, res) => {
  ...
  if (
    uploadedThumbnail &&
    existingCourse.thumbnail !== uploadedThumbnail.publicUrl
  ) {
...
exports.hide = asyncHandler(async (req, res) => {
  ...
  if (course.status === "Pending") {
    throw new AppError(
      409,
      "A pending course cannot be hidden until Admin reviews it.",
    );
  }
