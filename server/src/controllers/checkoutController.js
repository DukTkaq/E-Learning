...
const calculatePurchasedItems = (courses, coupon) =>
  courses.map((course) => {
    ...
    const amountMinor = Math.round(
      (baseAmount * (100 - discountPercent)) / 100,
    );
...
const hasSameCheckoutPricing = (payments, purchasedItems) => {
  if (payments.length !== purchasedItems.length) return false;

  const itemByCourseId = new Map(
    purchasedItems.map((item) => [String(item.course.id), item]),
  );

  return payments.every((payment) => {
    const item = itemByCourseId.get(String(payment.course_id));
    if (!item) return false;

    const sameAmount = amountToMinorUnits(payment.amount) === item.amountMinor;
    const sameCoupon = String(payment.coupon_id || "") === String(item.couponId || "");
    return sameAmount && sameCoupon;
  });
};
...
const loadCartForCheckout = async (userId, transaction) => {
  ...
  if (
    ...
    courses.some((course) => course.status !== "Approved")
  ) {
    throw new AppError(
      409,
      "One or more courses are no longer available. Refresh your cart.",
    );
  }
...
const createPendingPayments = async (userId, couponCode) =>
  sequelize.transaction(async (transaction) => {
    const { courses, courseIds } = await loadCartForCheckout(
      userId,
      transaction,
    );
    ...
    const totalMinor = purchasedItems.reduce(
    ...
    if (totalMinor <= 0) {
      throw new AppError(
        400,
        "VNPay requires a checkout total greater than 0 VND.",
      );
    }
    ...
    if (activePayments.length) {
      ...
      const references = new Set(
        activePayments.map((payment) => payment.checkout_ref),
      );
      const activeCourseIds = activePayments
      ...
      if (!sameCart) {
        throw new AppError(
          409,
          "Another VNPay checkout is still pending for one of these courses.",
        );
      }

      if (hasSameCheckoutPricing(activePayments, purchasedItems)) {
        return {
          checkoutRef: activePayments[0].checkout_ref,
          createdAt: activePayments[0].created_at,
          expiresAt: activePayments[0].expires_at,
          totalMinor: activePayments.reduce(
            (sum, payment) => sum + amountToMinorUnits(payment.amount),
            0,
          ),
        };
      }
      ...
      await Payment.update(
        ...
        {
          where: {
            checkout_ref: activePayments[0].checkout_ref,
            status: "Pending",
          },
          transaction,
        },
...
const processVnpayIpn = async (query, config) => {
  if (
    !verifySignature(query, config.hashSecret) ||
    query.vnp_TmnCode !== config.tmnCode
  ) {
  ...
  if (
    !CHECKOUT_REF_PATTERN.test(checkoutRef) ||
...
exports.vnpayReturn = async (req, res) => {
  ...
  try {
    ...
    signatureValid =
      verifySignature(req.query, config.hashSecret) &&
  ...
  redirectUrl.searchParams.set(
    "response_code",
    String(req.query.vnp_ResponseCode || ""),
  );
...
exports.__test = { hasSameCheckoutPricing };
