...
export default function CourseManagementPage() {
  ...
  const loadCourses = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Could not load course management.",
      );
      setCourses([]);
  ...
  const saveCourse = async (payload) => {
    ...
    try {
      if (editingCourse) {
        ...
        toast.success(
          "Course draft created. Add lessons and quizzes before submitting it.",
        );
      }
      setModalOpen(false);
      await loadCourses();
      ...
      toast.error(
        error.response?.data?.message || "Could not save the course.",
      );
    } finally {
  ...
  const submitForApproval = async (course) => {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Could not submit the course.";
  ...
  const hide = async (course) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Could not hide the course.",
      );
    }
  };
    ...
    <section>
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-2 text-gray-500">
            {summary.active} active course{summary.active === 1 ? "" : "s"} ·{" "}
            {summary.total} total
      ...
      <div ref={listRef} className="scroll-mt-24">
        ...
        <CourseTable
          ...
          emptyTitle={
            hasAppliedFilters ? "No matching courses" : "No courses yet"
          }
          emptyDescription={
            hasAppliedFilters
          ...
          onViewReviews={(course) =>
            navigate(`/instructor/courses/${course.id}/reviews`)
          }
          onEdit={openEdit}
          onHide={hide}
