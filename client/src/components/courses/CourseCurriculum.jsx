import { BookOpen, CheckCircle2, CircleHelp, Film, GraduationCap } from 'lucide-react';

const OPTION_KEYS = ['A', 'B', 'C', 'D'];

const getOptionText = (question, key) => question[`option_${key.toLowerCase()}`];
...
function QuestionList({ questions = [] }) {
  if (questions.length === 0) {
    return <p className="rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-700">No questions have been added to this quiz.</p>;
  }
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {OPTION_KEYS.map((key) => {
              ...
              return (
                ...
                >
                  {correct ? <CheckCircle2 className="mt-0.5 shrink-0" size={16} /> : <span className="font-bold">{key}.</span>}
...
function LessonCard({ lesson, index }) {
  ...
  return (
    <details className="group overflow-hidden rounded-2xl border border-slate-200 bg-white" open={index === 0}>
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="truncate font-bold text-slate-900">{lesson.title}</h3>
              {lesson.is_final && (
                <span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-700">Final lesson</span>
              )}
            </div>
            <p className="mt-1 text-sm text-slate-500">
              {quiz ? `1 quiz · ${questionCount} question${questionCount === 1 ? '' : 's'}` : 'Quiz not added'}
            </p>
          </div>
        </div>
        <span className="shrink-0 text-sm font-semibold text-primary group-open:hidden">Show</span>
        <span className="hidden shrink-0 text-sm font-semibold text-primary group-open:inline">Hide</span>
      </summary>

      <div className="border-t border-slate-100 bg-slate-50 p-5">
        <div className="mb-4 flex items-center gap-2 text-sm text-slate-500">
          <Film size={17} />
          <span className="truncate">{lesson.video_url || 'No video added'}</span>
        </div>

        {!quiz ? (
          <p className="rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-700">This lesson does not have a quiz yet.</p>
        ) : (
          <section>
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Quiz</p>
...
export default function CourseCurriculum({ lessons = [] }) {
  return (
    <section className="mt-6 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm sm:p-8">
      <div className="mb-6">
        <p className="text-sm font-bold uppercase tracking-wider text-primary">Course content</p>
        <h2 className="mt-1 text-2xl font-bold text-slate-900">Curriculum</h2>
        <p className="mt-2 text-slate-500">Review every lesson, quiz and answer before submitting the course.</p>
      </div>
