...
export default function CourseActionButtons({
  ...
  return (
    <div className="flex justify-end gap-2">
      ...
      <button type="button" onClick={() => onEdit(course)} disabled={!editable} title={readOnlyNotice?.title || 'Edit course'} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold text-primary hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-40">
        <Edit3 size={16} /> Edit
      </button>
      {canSubmit && (
        <button type="button" onClick={() => onSubmitForApproval(course)} disabled={submitting} className="inline-flex items-center gap-1.5 rounded-lg bg-primary/10 px-3 py-2 text-sm font-semibold text-primary hover:bg-primary/20 disabled:cursor-not-allowed disabled:opacity-50">
          {submitting ? <LoaderCircle className="animate-spin" size={16} /> : <Send size={16} />}
          Submit
        </button>
      )}
      <button type="button" onClick={() => onHide(course)} disabled={isHidden || isPending} title={isPending ? 'Course is under review' : 'Hide course'} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold text-error hover:bg-error/10 disabled:cursor-not-allowed disabled:opacity-40">
        <EyeOff size={16} /> Hide
      </button>
    </div>
  );
