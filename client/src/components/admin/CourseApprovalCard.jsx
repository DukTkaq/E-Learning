import { CheckCircle2, Image, Mail, UserRound, XCircle } from 'lucide-react';
import CourseStatusBadge from '../courses/CourseStatusBadge';
import { resolveAssetUrl } from '../../utils/assets';

const currency = new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' });

export default function CourseApprovalCard({ course, reviewing, onReview }) {
  return (
    <article className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
      <div className="grid md:grid-cols-[220px_1fr]">
        ...
        <div className="flex min-w-0 flex-col p-5">
          ...
          {course.status === 'Pending' && <div className="mt-auto flex justify-end gap-3 border-t border-gray-100 pt-4"><button type="button" disabled={reviewing} onClick={() => onReview(course, 'Rejected')} className="inline-flex items-center gap-2 rounded-xl bg-error/10 px-4 py-2.5 font-semibold text-error hover:bg-error/15 disabled:opacity-50"><XCircle size={18} /> Reject</button><button type="button" disabled={reviewing} onClick={() => onReview(course, 'Approved')} className="inline-flex items-center gap-2 rounded-xl bg-success px-4 py-2.5 font-semibold text-white shadow-lg shadow-success/20 hover:opacity-90 disabled:opacity-50"><CheckCircle2 size={18} /> Approve</button></div>}
        </div>
      </div>
    </article>
  );
