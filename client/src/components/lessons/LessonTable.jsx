...
export default function LessonTable({ lessons, loading, readOnly = false, hasFilters = false, onEdit, onDelete, onMoveUp, onMoveDown, onManageQuiz }) {
  if (loading) {
  ...
  if (!lessons.length) {
    return (
      <div className="rounded-2xl border border-dashed border-primary/25 bg-white p-12 text-center shadow-sm">
        ...
        <h3 className="font-bold text-slate-800">{hasFilters ? 'No matching lessons' : 'No lessons yet'}</h3>
        <p className="mt-1 text-sm text-gray-500">{hasFilters ? 'Try another keyword or filter.' : 'Create your first lesson to start building course content.'}</p>
      </div>
    );
  }
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-1">
                    {!readOnly && !lesson.is_final && (
                      <>
                        <button type="button" onClick={() => onMoveUp(lesson)} disabled={!lesson.can_move_up} className="rounded-lg p-2 text-gray-400 hover:bg-slate-100 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-30" title="Move up">
                          <ArrowUp size={16} />
                        </button>
                        <button type="button" onClick={() => onMoveDown(lesson)} disabled={!lesson.can_move_down} className="rounded-lg p-2 text-gray-400 hover:bg-slate-100 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-30" title="Move down">
                          <ArrowDown size={16} />
                        </button>
                      </>
                    )}
